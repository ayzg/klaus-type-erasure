...
struct IndirectShape : T, ShapeBaseCRTP<IndirectShape<T>> {
  ...
  std::string Format(this const SelfT& self) {
    if constexpr (ShapeHasMemberFormat<T>) {
      return static_cast<const ShapeBaseCRTP<IndirectShape<T>>&>(self)
                 .ShapeBaseCRTP_Format() +
             static_cast<const T&>(self).Format();
    } else if constexpr (ShapeHasStaticFormat<T>) {
      using ::Format;
      return static_cast<const ShapeBaseCRTP<IndirectShape<T>>&>(self)
                 .ShapeBaseCRTP_Format() +
             Format(static_cast<const T&>(self));
    } else {
  ...
  class Model : public Interface {
   ...
   public:
    ...
    constexpr int Calculate() const override {
      if constexpr (std::is_base_of_v<ShapeBaseCRTP<T>, T>) {
        return static_cast<const ShapeBaseCRTP<T>&>(object_.self()).Calculate();
      } else if constexpr (ShapeHasMemberCalculate<T>) {
        return object_.Calculate();
      } else if constexpr (ShapeHasStaticCalculate<T>) {
        using ::Calculate;
        return Calculate(object_.self());
      } else {
...
// style class. We should be able to use the Bat Class - or actually
// any class at that point - since the base provides all default impls.
// And it wil ldraw the position, and Bat format without inheriting
// from any base. This can be seen as a 'mix-in' pattern.
